...
public class LogFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		eventReceiver=new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				if(logTextView!=null){
					logTextView.setText(((AccessSupporterApplication)getActivity().getApplication()).getLogString());
				}
			}
		};
		eventFilter=new IntentFilter();
		eventFilter.addAction(AccessSupporterService.STATUS_CHANGED);
		getContext().registerReceiver(eventReceiver,eventFilter);
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		logTextView=(TextView)view.findViewById(R.id.log);
		logTextView.setText(((AccessSupporterApplication)getActivity().getApplication()).getLogString());
